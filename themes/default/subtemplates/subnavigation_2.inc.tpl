...
{if $categories && $mode=='index'}
    <form id="form_categories" action="index.php" method="get" accept-charset="{#charset#}">
        <div>
            <input type="hidden" name="mode" value="{$mode}" />
            <select size="1" name="category" title="{#category_title#}">
                <option value="0"{if $category==0} selected="selected"{/if}>{#all_categories#}</option>
                {if $category_selection}
                    <option value="-1"{if $category==-1} selected="selected"{/if}>{#my_category_selection#}</option>
                {/if}
            ...
            <!--<noscript><div class="inline"><input type="submit" value="&raquo;" title="{#go#}" /></div></noscript>-->
        </div>
    </form>
    <div id="category_menu">
        <p><a href="">Categories <span>&#9661;</span></a></p>
        <ul aria-expanded="false" class="loop_bord">
            <li><a href="" data-category="0}">Show All Categories</a></li>
            {foreach key=key item=val from=$categories}
                {if $key!=0}
                    <li class="cat_{$key}"><a href="" data-category="{$key}">{$val}</a></li>
                {/if}
            {/foreach}
        </ul>
    </div>
...
{elseif $mode=='entry'}
    <ul id="subnavmenu">
        <li><a class="openthread" href="index.php?mode=thread&amp;id={$tid}#p{$id}" title="{#open_in_thread_linktitle#}">{#open_in_thread_link#}</a></li></ul>
{elseif $mode=='thread'}
    <ul id="subnavmenu">
...
{/if}
{if $pagination_top}
    {if $pagination_top.previous}
        <a href="index.php?mode={$mode}&amp;page={$pagination_top.previous}{if $category}&amp;category={$category}{/if}"><img class="previous" src="{$THEMES_DIR}/{$theme}/images/plain.png" alt="[&laquo;]" title="{#previous_page_link_title#}" width="6" height="11" /></a>
    {/if}
    <form action="index.php" method="get">
        <div class="inline">
            ...
            <select  size="1" name="page" title="{#browse_page_title#}">
                {foreach from=$pagination_top.items item=item}
                    {if $item!=0}
                        <option value="{$item}"{if $item==$page} selected="selected"{/if}>{$item}</option>
                    {/if}
                {/foreach}
            </select>
            <noscript><div class="inline"><input type="submit" value="&raquo;" title="{#go#}" /></div></noscript>
    ...
    {if $pagination_top.next}
        <a href="index.php?mode={$mode}&amp;page={$pagination_top.next}{if $category}&amp;category={$category}{/if}"><img class="next" src="{$THEMES_DIR}/{$theme}/images/plain.png" alt="[&raquo;]" title="{#next_page_link_title#}" width="6" height="11" /></a>
    {/if}
{/if}
