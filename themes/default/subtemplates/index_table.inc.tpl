...
<td><p>{if $data.$thread.user_type==2}<span class="admin" title="{#administrator_title#}">{$data.$thread.name}</span>{elseif $data.$thread.user_type==1}<span class="mod" title="{#moderator_title#}">{$data.$thread.name}</span>{else}{$data.$thread.name}{/if}</p></td>
<td><p class="format_date">{$data.$thread.formated_time}</p></td>
...
{if $categories && $category<=0}
    <td>
        <p>{if $data.$thread.category_name}<a href="index.php?mode=index&amp;category={$data.$thread.category}" title="{#change_category_link#|replace:"[category]":$data.$thread.category_name|escape:"html"}"><span class="category">{$data.$thread.category_name}</span></a>{else}&nbsp;{/if}</p>
    </td>
{/if}
</tr>
{/foreach}
