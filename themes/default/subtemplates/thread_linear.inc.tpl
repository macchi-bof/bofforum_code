...
<div class="header">
{if $element.avatar}<div class="avatar_container avatar_small"><img id="avatar-{$element.id}" class="avatar" src="{$element.avatar.image}" alt="{#avatar_img_alt#}" width="100%" {*width="{$element.avatar.width}" height="{$element.avatar.height}"*} /></div>{/if}
...
<div class="reply">
    {if $element.locked==0}
        <p><a class="stronglink" href="index.php?mode=posting&amp;id={$element.id}&amp;back=thread" title="{#reply_link_title#}">{#reply_link#}</a>{else}<span class="locked">{#posting_locked#}</span></p>
    {/if}
</div>
{if $user_id > 0}
    {if $pid == 0}
...
{if $element.options}
<ul class="options">
    {if $element.views}<li class="views">{if $element.views==1}{#one_view#}{else}{#several_views#|replace:"[views]":$element.views}{/if}</li>{/if}
