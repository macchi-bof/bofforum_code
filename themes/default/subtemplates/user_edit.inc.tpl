...
<tr>
    ...
    <td class="d"><p><strong>{$user_name}</strong></p></td>
</tr>
{if $settings.avatars>0}
<tr>
    <td class="c"><p><strong>{#edit_avatar#}</strong></p></td>
    <td class="d">
        ...
        <p>[ <a id="edit_avatar" href="index.php?mode=avatar">{#edit_avatar_link#}</a> ]</p>
    </td>
</tr>
{/if}
<tr>
    <td class="c"><p><strong>{#edit_user_pw#}</strong></p></td>
    <td class="d"><p>[ <a href="index.php?mode=user&amp;action=edit_pw">{#edit_pw#}</a> ]</p></td>
</tr>
<tr>
    <td class="c"><p><strong>{#edit_user_email#}</strong></p></td>
    <td class="d"><p><!--<a href="mailto:{$user_email}">-->{$user_email}<!--</a>--> | [ <a href="index.php?mode=user&amp;action=edit_email">{#edit_email#}</a> ]<br />
        <input id="email_contact" type="checkbox" name="email_contact" value="1"{if $email_contact==1} checked="checked"{/if} /><label for="email_contact">{#edit_user_email_contact#}</label></p></td>
</tr>
<tr>
...
<tr>
    ...
    <td class="d">
        ...
        <input id="female" type="radio" name="user_gender" value="2"{if $user_gender=="2"} checked="checked"{/if} />
        <label for="female">{#female#}</label>
    </td>
</tr>
<tr>
    <td class="c"><label for="user_birthday"><strong>{#edit_user_birthday#}</strong></label></td>
    <td class="d"><input id="user_birthday" type="text" size="40" name="user_birthday" value="{$user_birthday}" /> <span>({#birthday_format#})</span></td>
</tr>
<tr>
...
{if $categories}
<tr>
    <td class="c"><p><strong>{#edit_user_cat_selection#}</strong></p></td>
    <td class="d">
...
<tr>
    ...
    <td class="d">
        ...
        {/if}
        <p><label for="user_time_difference">{#edit_user_time_difference#}</label><br /><input id="user_time_difference" type="text" size="6" name="user_time_difference" value="{$user_time_difference}" maxlength="6" /></p>
    </td>
</tr>
...
<tr>
    <td class="c"><p><strong>{#edit_user_auto_login#}</strong></p></td>
