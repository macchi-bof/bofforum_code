...
<div class="header">
{if $data.$element.avatar}<div class="avatar_container avatar_small"><img id="avatar-{$data.$element.id}" class="avatar" src="{$data.$element.avatar.image}" alt="{#avatar_img_alt#}" width="100%" {*width="{$data.$element.avatar.width}" height="{$data.$element.avatar.height}"*} /></div>{/if}
...
<div class="reply">
    {if $data.$element.locked==0}
        <p><a class="stronglink" href="index.php?mode=posting&amp;id={$data.$element.id}&amp;back=thread" title="{#reply_link_title#}">{#reply_link#}</a>{else}<span class="locked">{#posting_locked#}</span></p>
    {/if}
</div>
{if $user_id > 0}
    {if $pid == 0}
...
<ul class="options">
    {if $data.$element.views}<li class="views">{if $data.$element.views==1}{#one_view#}{else}{#several_views#|replace:"[views]":$data.$element.views}{/if}</li>{/if}
