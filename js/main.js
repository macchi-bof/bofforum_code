...
/************************ MyLittleForum-Objekte *************************************/
	...
	function MyLittleJavaScript() {
		...
		var createLikesSpan = function(id) {
			var outer = document.createElementWithAttributes("span", {"hidden": true, "className":"likeclass"},null);
		...
		var setPreviewBoxToProfil = function(el) {
			...
			if (pid && el.parentNode) {
				...
				el.parentNode.appendChild( createLikesSpan(pid) );
			}
		};
		...
		var setPreviewBoxToReplyPage = function(el) {
			...
			if (pid) {
				...
				el.appendChild( createLikesSpan(pid) );
			}
		};
			...
			for (var i=0; i<els.length; i++) {
				...
				if (pLink && ajaxPreviewWindow) {
					if (links.length >= 1) {
						...
						el.insertBefore(createLikesSpan(pid), link);
		...
		this.likeThread = function(id, db) {
			var e = document.getElementById("lk"+id);
			var e2 = document.getElementById("lks"+id);
			if (db == 1) {
				var querys = [
					new Query("mode", "like"),
					new Query("like", e.lk),
				...
				new Request(strURL, "POST", querys, this, "updateLike", null, true);
			}
			if (e.lk == "1") {
				e.innerHTML = "&nbsp;Unlike&nbsp;";
				e.lk = 0;
				this.computeLikes(e2, 1);
			}
			else {
				e.innerHTML = "&nbsp;Like&nbsp";
				e.lk = 1;
				this.computeLikes(e2, -1);
			}
			return false;
		...
		*/
		...
		this.computeLikes = function(e, val) {
			...
			if (num == 1) {
				e.innerHTML = num + " like";
				e.style.color = "red";
				return;
			}

			e.innerHTML = num + " likes";
			
		}

		this.updateAllLikes = function(xml) {
			var u = xml.getElementsByTagName("self");
                        var meid = u[0].firstChild.data;
			if (meid == "anon") {
				return;
			}

			var ts = xml.getElementsByTagName("like");
			var n = ts.length;
			for (i = 0; i < n; i++) {
				var tid = ts[i].childNodes[0].firstChild.data;
				var e = document.getElementById("lk"+tid);
				var uid = ts[i].childNodes[1].firstChild.data;
				if (uid == meid) {
					e.innerHTML = "&nbsp;Unlike&nbsp;";
					e.lk = 0;
				}
				var e = document.getElementById("lks"+tid);
				e.style.color = "red";
				this.computeLikes(e, 1);
			}
			var lkes = document.getElementsByClassName("likeclass");
			n = lkes.length;
			for (i = 0; i < n; i++) {
				var e = lkes.item(i);
				e.hidden = false;
		...
		this.getMyLikedThreads = function() {
			var querys = [
				new Query("mode", "like"),
				new Query("id", "all")
			];
			new Request(strURL, "POST", querys, this, "updateAllLikes", null, true);
		}
